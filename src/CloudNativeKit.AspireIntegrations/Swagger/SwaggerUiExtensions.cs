...
public static class SwaggerUIExtensions
{
    ...
    )
    {
        if (!builder.ApplicationBuilder.Resources.OfType<SwaggerUIResource>().Any())
        {
            ...
            builder
                .ApplicationBuilder.AddResource(new SwaggerUIResource("swagger-ui"))
                .WithInitialState(
        ...
        return builder.WithAnnotation(
            new SwaggerUIAnnotation(documentNames ?? ["v1"], path, builder.GetEndpoint(endpointName))
        );
    }

    private class SwaggerUiHook(

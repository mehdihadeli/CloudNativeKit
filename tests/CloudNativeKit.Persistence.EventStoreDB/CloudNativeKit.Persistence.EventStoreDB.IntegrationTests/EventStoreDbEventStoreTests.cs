...
public class EventStoreDbEventStoreTests : IClassFixture<IntegrationFixture>
{
    ...
    public async Task append_multiple_and_get_stream_events_with_version_should_return_correct_events()
    {
        ...
        shoppingCart.DequeueUncommittedDomainEvents();

        var streamEvents = uncommittedEvents
            .Select(x =>
                StreamEventEnvelopeFactory.From(
                    x,
                    new StreamEventMetadata(x.EventId.ToString(), (ulong)x.AggregateSequenceNumber, null)
                )
            )
            .ToImmutableList();
        ...
        events.Last().Should().BeOfType<IStreamEventEnvelope<ShoppingCartConfirmed>>();
    ...
    public async Task append_single_to_existing_stream_and_get_stream_events_with_version_should_return_correct_events()
    {
        ...
        var defaultAggregateState = new ShoppingCart { Id = shoppingCart.Id };
        ...
        aggregate.DequeueUncommittedDomainEvents();
    ...
    private class ShoppingCart : EventSourcedAggregate<Guid>
    {
        ...
        {
            var shoppingCart = new ShoppingCart { Id = Guid.NewGuid() };

            shoppingCart.ApplyEvent(new ShoppingCartInitialized(shoppingCart.Id, clientId));
